function [psi_opt,phi_in,phi_out] = wavefront_correction(x,y,x_zone,y_zone,r_update,k,Z,FOM)
    ...
    [~,nz_in_ind] = find(r_update ~= 0); nz_in_ind = unique(nz_in_ind);
    ...
    while delta_M > 0.1
        ...
        [~,nz_in_ind] = find(r_in_update ~= 0); nz_in_ind = unique(nz_in_ind);
        for iii = 1:length(nz_in_ind)
        ...
        [~,nz_out_ind] = find(r_out_update ~= 0); nz_out_ind = unique(nz_out_ind);
        for iii = 1:length(nz_out_ind)
